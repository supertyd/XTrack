...
# network related
from lib.models.xtrack import build_ostrack
...
def run(settings):
    settings.description = 'Training script for xtrack'
    ...
    # Create network
    if settings.script_name == "xtrack":
        net = build_xtrack(cfg)
    else:
    ...
    if settings.script_name == "xtrack":
        ...
        actor = XTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
