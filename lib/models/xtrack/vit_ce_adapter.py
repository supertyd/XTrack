...
class VisionTransformerCE(VisionTransformer):
        ...
        for i in range(depth):
            ...
            if i < 20:
                ...
                CEABlock(
                        ...
                        keep_ratio_search=ce_keep_ratio_i,patch_num= self.num_patches_search+self.num_patches_template,)
                )
            else:
                blocks.append(
                    DSBlock(
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        if self.pos_embed_x.shape[1] == 576:
        ...
        removed_flag = False
        loss_prompt_balance = 0
        logits_prompt = []
        for i, blk in enumerate(self.blocks):
           
            x, global_index_t, global_index_s, removed_index_s, attn, \
            xi, global_index_ti, global_index_si, removed_index_si, attn_i,loss_balance, logits_attn, logits_ffn = \
                blk(x, xi, global_index_t, global_index_ti, global_index_s, global_index_si, mask_x, ce_template_mask,
                    ce_keep_rate)
            loss_prompt_balance = loss_prompt_balance + loss_balance
            logits_prompt.append(logits_attn)
            logits_prompt.append(logits_attn)
        ...
        aux_dict = {
            ...
            "loss_prompt_balance": loss_prompt_balance,
            'logits_all': logits_prompt
        }
