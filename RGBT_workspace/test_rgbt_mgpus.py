...
import lib.test.parameter.xtrack as rgbt_adapter_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter'):
    ...
    if script_name == 'xtrack':
        params = rgbt_adapter_params.parameters(yaml_name, epoch)
        mmtrack = XTrack(params)  # "GTOT" # dataset_name
        tracker = XTrack_RGBT(tracker=mmtrack)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='xtrack', help='Name of tracking method(ostrack, adapter, ftuning).')
    ...
    if dataset_name == 'GTOT':
        ...
        seq_home = '/home/zwu-guest123/RGBXTestDatasets/RGBT234/RGBT234'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
        ...
        seq_home = '/home/zwu-guest123/RGBXTestDatasets/LasHeR/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
